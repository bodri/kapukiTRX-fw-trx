 ...
 */
imu::Quaternion BNO055::getQuaternion() {
	uint8_t buffer[8];
	memset(buffer, 0, 8);
 ...
 *  @return always returns true
 */
//bool Bno055::getEvent(sensors_event_t *event, adafruit_vector_type_t vec_type)
//{
//  /* Clear the event */
//  memset(event, 0, sizeof(sensors_event_t));
//
...
//  _wire->endTransmission();
	...
	if (HAL_I2C_Mem_Write(&hi2c1, i2cAddress, reg, 1, &value, 1, 1000)
			== HAL_OK) {
	}
...
//#endif
	...
	if (HAL_I2C_Mem_Read(&hi2c1, i2cAddress, reg, 1, &value, 1, 1000)
			== HAL_OK) {
	}
...
//  }
	...
	if (HAL_I2C_Mem_Read(&hi2c1, i2cAddress, reg, 1, buffer, len, 1000)
			== HAL_OK) {
	}
